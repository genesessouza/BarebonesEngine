...
sdf_raster_test::sdf_raster_test()
{
	// -------------------------------- SCENE ENTITIES ---------------------------
	{
		// CAMERA
		{
			camera = new perspective_camera(45.0f, 800.0f, 600.0f, 0.1f, 70.0f);
		...
		// FLOOR
		{
			floor.set_rotation(glm::vec3(0, 0, 0));
		...
		// RED CUBE
		{
			...
			cube1.get_material()->set_diffuse(0.5f);
			cube1.get_material()->set_specular(0.5f);
		}

		// BLUE CUBE
		{
			cube2.set_position(glm::vec3(-3, 0.65, 3));

			cube2.get_material()->set_color(glm::vec4(0.2, 0.3, 0.7, 1));
			cube2.get_material()->set_diffuse(0.5f);
			cube2.get_material()->set_specular(0.5f);
		}

		// GREEN CUBE
		{
			cube3.set_position(glm::vec3(3, -2, 1));
			cube3.set_rotation(glm::vec3(-135, 0, 0));
			cube3.set_scale(glm::vec3(2, 1, 3));
	...
	// ---------------------------------- RENDERING & DEBUG -----------------------------
	{
		...
		m_rendering_layer->add_object(cube2, false);
		m_rendering_layer->add_object(cube3, false);
