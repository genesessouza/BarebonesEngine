...
void shader::create(const std::string& vertex_source, const std::string& fragment_source)
{
	...
	glGetProgramiv(renderer_id, GL_LINK_STATUS, (int*)&is_linked);
	if (is_linked == GL_FALSE)
	{
		int message_length = 0;
		glGetProgramiv(renderer_id, GL_INFO_LOG_LENGTH, &message_length);

		char* message = (char*)_malloca(message_length * sizeof(char));
		glGetProgramInfoLog(renderer_id, message_length, &message_length, &message[0]);
...
std::shared_ptr<shader> shader::instantiate(const char* shader_filepath)
{
	return std::make_shared<shader>(shader_filepath);
}

shader::shader(const std::string& shader_filepath) : shader_filepath(shader_filepath)
{
...
void shader::get_shader_info_log(const std::string& uniform_name) const
{
	...
	glGetShaderInfoLog(renderer_id, message_length, &message_length, message);
...
void shader::get_float(const std::string& name)
{
	...
	glUseProgram(renderer_id);
	glGetUniformfv(renderer_id, location, &value);
}
...
glm::vec3 shader::get_vec3(const std::string& name)
{
	...
	glUseProgram(renderer_id);
	glGetUniformfv(renderer_id, location, vector);
...
glm::mat3 shader::get_mat3(const std::string& name)
{
	...
	float matrix[9];
	glUseProgram(renderer_id);
	glGetUniformfv(renderer_id, location, matrix);
